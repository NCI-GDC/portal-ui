...
import { isEqual, head, includes } from 'lodash';
...
const FrequentlyMutatedGenesTableComponent = compose(
  ...
  withPropsOnChange(
    ...
    ({ relay, query, defaultFilters }) => {
      relay.setVariables({
        fetchData: true,
        fmgTable_offset: parseIntParam(query.fmgTable_offset, relay.route.params.fmgTable_offset || 0),
  ...
  projectIds = [],
  cohort: { genes, cases, filteredCases, ssms },
  relay,
  ...
  const projectId = head(projectIds) || '';
  return (
    <Loader
      loading={!genes}
      height="387px"
        ...
        data={data.map(g => ({
          ...
          num_affected_cases_project: (
            <span>
              ...
              <span>&nbsp;({((g.score / numCasesAggByProject[projectId]) * 100).toFixed(2)}%)</span>
            </span>
          ),
          num_affected_cases_cohort: (
            <span>
              ...
              <span>&nbsp;({((g.score / filteredCases.hits.total) * 100).toFixed(2)}%)</span>
            </span>
          ),
          ...
          num_mutations: (
            <MutationsCount
              ...
              ssms={ssms}
              filters={makeFilter([
                { field: 'consequence.transcript.gene.gene_id', value: [g.gene_id] },
                ...(projectIds.length ? [{ field: 'ssms.occurrence.case.project.project_id', value: projectIds }] : []),
              ], false)}
            />
          ),
          survival_plot: (
