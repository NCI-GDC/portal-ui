...
import entities from '@ncigdc/entities';
...
export const SearchTable = compose(
  ...
  const tableInfo = entities[entityType].table
    .slice()
    ...
    <div>
      <Row style={{ backgroundColor: 'white', padding: '1rem', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <TableActions
          ...
          downloadFields={entities[entityType].table.filter(x => x.downloadable).map(x => x.id)}
          sortOptions={entities[entityType].table.filter(x => x.sortable)}
        />
      </Row>
