...
import { uniq, get, sortBy } from 'lodash';
...
const Mutation = compose(
  ...
  const consequenceData = consequences.reduce((acc, c) => {
    const { gene: { gene_id }, transcript_id: transcriptId } = c.node.transcript;
    ...
    const canonicalOnly = c.node.transcript.is_canonical
      ? {
        ...
        canonical_transcript_id: transcriptId,
      }
      : {};
  ...
  const consquenceDataMapped = Object.values(consequenceData)
    .map(d => ({
      ...
      transcripts: (
        <CollapsibleList
          style={{ width: '180px' }}
          data={sortBy(d.transcripts, t => t !== d.canonical_transcript_id).map(t => (
            <span>
              <ExternalLink
                key={t}
                style={{
              ...
              {t === d.canonical_transcript_id && <BubbleIcon text='C' toolTipText='Canonical' backgroundColor={theme.primary} />}
            </span>
          ))}
        />
      ),
