    ...
    <LocationSubscriber>
      {
        (ctx: {| pathname: string, query: TRawQuery |}) => (
          <Project
            viewer={viewer}
            query={ctx.query}
            clinicalCount={viewer.cohort.cases.hits.total}
            biospecimenCount={viewer.repository.cases.hits.total}
  ...
  fragments: {
    viewer: () => Relay.QL`
      fragment on Root {
        ...
        mostAffectedCasesChartFragment: cohort {
          ${MostAffectedCasesChart.getFragment('cohort')}
        }
        mostAffectedCasesTableFragment: cohort {
          ${MostAffectedCasesTable.getFragment('cohort')}
        }
        frequentlyMutatedGenesChartFragment: cohort {
          ${FrequentlyMutatedGenesChart.getFragment('cohort')}
        }
        frequentlyMutatedGenesTableFragment: cohort {
          ${FrequentlyMutatedGenesTable.getFragment('cohort')}
        }
        geneProjectBreakdownFragment: cohort {
          ${GeneProjectBreakdown.getFragment('cohort')}
        }
        repository {
          cases {
            hits(first: 0, filters: $biospecimenFilters){
              total
            }
          }
        }
        cohort {
          cases {
            hits(first: 0, filters: $clinicalFilters){
              total
            }
