...
const enhance = compose(
  withRouter,
  withPropsOnChange(['query'], ({ query: { filters } }) => ({ filters: JSURL.parse(filters) })),
  ...
  <Info style={style}>
    ...
    {linkPathname && (!hideLinkOnEmpty || !!currentFilters.length) &&
      <LinkButton
        ...
        query={currentFilters.length && {
          filters: makeFilter(currentFilters.map(({ content: { field, value } }) =>
                                                 ({ field: linkFieldMap(field), value })), false),
        }}
      >
