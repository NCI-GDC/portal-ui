  ...
  return (
    <TogglableUl
      onToggle={() => {
        relay.setVariables({
          fetchData: true,
          filters: makeFilter([{ field: 'genes.gene_id', value: id }], false),
        });
        setLoading(l => !l);
      }}
      items={[
        <span key="total">
          <CohortLink
            query={{
              ...
              filters: makeFilter([
                { field: 'genes.gene_id', value: [id] },
                { field: 'cases.available_variation_data', value: ['ssm'] },
              ], false),
            }}
          >
            {allCases.hits.total.toLocaleString()}
          </CohortLink>
          <span> / </span>
          <CohortLink
            query={{
          ...
          <span>&nbsp;({((allCases.hits.total / cases.hits.total) * 100).toFixed(2)}%)</span>
        </span>,
