...
async function getGenes({
  ...
  const filters = currentFilters.content.length ? currentFilters : {
    ...
    content: [
      ...
      { op: 'in', content: { field: 'case.ssm.consequence.transcript.annotation.impact', value: impacts } },
      { op: 'in',
        content: {
          field: 'case.ssm.consequence.transcript.consequence_type',
...
async function getOccurences({
  ...
  const filters = currentFilters.content.length ? currentFilters : {
    ...
    content: [
      ...
      { op: 'in', content: { field: 'ssm.consequence.transcript.annotation.impact', value: impacts } },
      {
        op: 'in',
...
async function getCases({
  ...
  const filters = currentFilters.content.length ? currentFilters : {
    ...
    content: [
      ...
      { op: 'in', content: { field: 'gene.ssm.consequence.transcript.annotation.impact', value: impacts } },
      projectId && { op: '=', content: { field: 'project.project_id', value: projectId } },
    ].filter(Boolean),
  };

  const r = await fetch(`${api}analysis/oncogrid/cases`, {
    method: 'POST',
