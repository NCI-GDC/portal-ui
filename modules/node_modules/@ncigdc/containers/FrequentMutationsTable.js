...
import { startCase, truncate, isEqual, get, includes } from 'lodash';
...
const mapData = (data: Array<Object>, projectId: string, shouldShowGeneSymbol: boolean, theme): Array<Object> => (
  data.map((hit) => {
    ...
    return {
      ...
      num_affected_cases_by_project: hit.allOccurrences.hits.edges.reduce((acc, { node: { case: { project } } }) => ({
        ...acc,
        [project.project_id]: acc[project.project_id]
          ? acc[project.project_id] + 1
          : 1,
      }), {}),
      impact,
...
const FrequentMutationsTableComponent = compose(
  ...
  cohort: { ssms, cases, filteredCases },
          ...
          }] : []),
          {
            ...
            title: (
              <Tooltip
                ...
                style={tableToolTipHint()}
              >
                # Affected Cases<br /> Across {projectId ? 'all Projects' : 'GDC'}
              </Tooltip>
            ),
          },
          {
        ...
        data={frequentMutations.map(x => ({
          ...
          ...(projectId ? { filteredOccurrences: (
            <span>
              ...
              >
                {x.filteredOccurrences.toLocaleString()}
              </CohortLink>
              <span> / </span>
              <CohortLink
                merge
              ...
              <span>&nbsp;({((x.filteredOccurrences / numCasesAggByProject[projectId]) * 100).toFixed(2)}%)</span>
            </span>
          ) } : {}),
          num_affected_cases_cohort: (
            <span>
              ...
              >
                {filteredCases.hits.total.toLocaleString()}
              </CohortLink>
              <span>&nbsp;({((x.score / filteredCases.hits.total) * 100).toFixed(2)}%)</span>
            </span>
          ),
          allOccurrences: (
            <TogglableUl
              items={[
                <span key="total">
                  ...
                  >
                    {x.allOccurrences.toLocaleString()}
                  </CohortLink>
                  <span> / </span>
                  <CohortLink
                    query={{
                  ...
                  <span>&nbsp;({((x.allOccurrences / cases.hits.total) * 100).toFixed(2)}%)</span>
                </span>,
                ...Object.entries(x.num_affected_cases_by_project) // eslint-disable-line fp/no-mutating-methods
                  ...
                  .map(([k, v]) => (
                    <span key={k}>
                      ...
                      >
                        {(numCasesAggByProject[k] || 0).toLocaleString()}
                      </CohortLink>
                      <span>&nbsp;({((v / numCasesAggByProject[k]) * 100).toFixed(2)}%)</span>
                    </span>
                  )),
              ]}
            />
    ...
    cohort: () => Relay.QL`
      fragment on Cohort {
        ...
        filteredCases: cases @include(if: $fetchData) {
          hits(first: 0 filters: $fmTable_filters) {
            total
          }
        }
