...
import { getFilterValue, makeFilter, replaceFilters } from '@ncigdc/utils/filters';
import { consequenceTypes, colorMap } from '@ncigdc/utils/filters/prepared/significantConsequences';
...
const OncoGridWrapper = compose(
  ...
  mapProps(({ title, impacts, ...props }) => {
    const cases = props.oncoGridData ? props.oncoGridData.cases.length : MAX_CASES;
    const genes = props.oncoGridData ? props.oncoGridData.genes.length : MAX_GENES;
    ...
    const currentConsequenceTypes = get(getFilterValue({
      currentFilters: props.currentFilters.content,
      dotField: 'ssms.consequence.transcript.consequence_type',
    }), 'content.value', consequenceTypes);
    const filteredConsequenceTypes = consequenceTypes.filter((c: any) => currentConsequenceTypes.includes(c));
      ...
      const gridParams = oncoGridParams({
          ...
          push({
            ...
            query: {
              filters: JSURL.stringify(
                makeFilter([
                  { field: 'cases.case_id', value: data.id },
                ], false)
              ),
            },
          });
        ...
        gridClick: (data: { id: string }) => {
          push({
            pathname: '/exploration',
            query: {
              filters: JSURL.stringify(makeFilter([{ field: 'ssms.ssm_id', value: data.id }], false)),
              facetTab: 'mutations',
        ...
        geneHistogramClick: (data: { id: string }) => {
          push({
            ...
            query: {
              filters: JSURL.stringify(
                makeFilter([
                  { field: 'genes.gene_id', value: data.id },
                ], false)
              ),
              facetTab: 'genes',
      ...
      {oncoGridData && !isLoading &&
        <Row style={{ marginLeft: 0, minHeight: '70px' }}>
          <div style={{ flexGrow: 1 }} className="oncogrid-legend">
            {heatMapMode ? <StepLegend rightLabel="More Mutations" /> : <SwatchLegend colorMap={colorMap} />}
          </div>
