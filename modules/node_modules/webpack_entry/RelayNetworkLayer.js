...
// This is just the Default Network Layer with an md5 hash of the body added to the Query requests
...
class RelayDefaultNetworkLayer {
  ...
  _sendQuery(request: RelayQueryRequest): Promise<any> {
    const hash = md5([request.getQueryString(), JSON.stringify(request.getVariables())].join(':'));
    return fetchWithRetries(`${this._uri}?hash=${hash}`, {
      ...this._init,
      body: JSON.stringify({
        query: request.getQueryString(),
        variables: request.getVariables(),
