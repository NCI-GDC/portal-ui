...
function legendGradient(track): string {
  return `
    <b>${track.name}: </b> ${track.min || 0} ${
      fadeSteps.reduce((html, opacity) => html + getSquare({ ...track, opacity }), '')
    } ${track.max}
  `;
}
...
function legendMulti(track): string {
  ...
  const values = typeof track.color === 'object' ? Object.keys(track.color) : track.values;

  return `
    <b>${track.name}:</b>${joinCharacter}${
      values.map(value => `
