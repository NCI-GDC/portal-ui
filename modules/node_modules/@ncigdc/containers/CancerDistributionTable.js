...
const CancerDistributionTableComponent = compose(
  ...
  .project__project_id.buckets.map(b => {
    ...
    return {
      ...
      num_affected_cases:
        `${b.doc_count} / ${totalCasesByProject.toLocaleString()} (${(freq * 100).toFixed(2)}%)`,
      ...geneId && { num_mutations: <MutationsCount
        key={b.key}
        ssms={cohort.ssms}
        filters={makeFilter([
          { field: 'consequence.transcript.gene.gene_id', value: [geneId] },
  ...
  const mutationsHeading = geneId ? [{ key: 'num_mutations',
              title: (
                <Tooltip
                Component={`Number of SSM observed in ${entityName}`}
        ...
        <EntityPageHorizontalTable
          headings={[
            ...
            { key: 'num_affected_cases',
              title: (
                <Tooltip
                  Component={`Number of Cases where ${entityName} contains SSM`}
                  style={tableToolTipHint()}
                >
                  # Affected Cases
                </Tooltip>
