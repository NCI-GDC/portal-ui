...
function getGenes({ relay, projectIds }: TProps): void {
  relay.setVariables({
    fetchGeneData: true,
    fmgChart_filters: projectIds.length ? {
      op: 'AND',
      content: [
        projectIds.length && {
          op: 'in',
...
const ProjectsChartsComponent = compose(
  ...
  withProps({
    async fetchData(props: TProps): Promise<*> {
      const topGenesSource = (props.cohort.genes.hits.edges || []).map(g => g.node);
      ...
      const numUniqueCases = props.cohort.cases.hits.total;
  ...
  lifecycle({
    ...
    componentWillReceiveProps(nextProps: TProps): void {
      if (!isEqual(this.props.projectIds, nextProps.projectIds)) {
        ...
        getGenes(nextProps);
      } else if (nextProps.cohort && !isEqual(this.props.cohort, nextProps.cohort)) {
        this.props.setState(s => ({ ...s, genesIsFetching: true }));
  ...
  const primarySiteProjects = projects.slice()
    .sort((a, b) => {
      // sort the projects with top mutated genes to the top so they're colored darker
      const aInArr = projectsInTopGenes.includes(a.project_id);
      const bInArr = projectsInTopGenes.includes(b.project_id);

      if (aInArr && !bInArr) return -1;
      if (!aInArr && bInArr) return 1;
      ...
      <Column style={{ paddingRight: '10px', minWidth: '550px', flexGrow: '2', flexBasis: '66%' }}>
        ...
        {!genesIsFetching ? [
          <div style={{ alignSelf: 'center', color: theme.greyScale7, fontSize: '1.2rem' }} key='bar-subtitle'>
            {`${numUniqueCases.toLocaleString()} Unique Case${numUniqueCases === 0 || numUniqueCases > 1 ? 's' : ''} with Mutation Data`}
          </div>,
...
export const ProjectsChartsQuery = {
  ...
  fragments: {
    cohort: () => Relay.QL`
      fragment on Cohort {
        cases { hits(first: 0 filters: $fmgChart_filters) { total } }
        genes {
          h: hits {
            total
          }
