...
const getAnnotationsLink = (annotations, entity) => {
  ...
  if (filteredAnnotations.length === 1) {
    return (<AnnotationLink id={filteredAnnotations[0].node.annotation_id}>
...
const File = ({
  ...
  const filteredAE = node.associated_entities.hits.edges
    ...
    .map(({ node: ae }) => ({
      ...
      annotation_count: getAnnotationsLink(node.annotations, ae),
    }));
