...
type TMapData = (data: Array<Object>, projectId: string, shouldShowGeneSymbol: boolean, theme: Object) => Array<Object>;

const mapData: TMapData = (data, projectId, shouldShowGeneSymbol, theme) => (
  data.map(hit => {
    ...
    >
      ...
      <EntityPageHorizontalTable
        ...
        data={frequentMutations.map(({ score = 0, ...x }) => ({ // eslint-disable-line
          ...x,
          mutation_uuid: <MutationLink uuid={x.ssm_id}>{x.ssm_id.substr(0, 8)}</MutationLink>,
          genomic_dna_change: (
            <Tooltip Component={<div style={{ maxWidth: 300, wordBreak: 'break-all' }}>{x.genomic_dna_change}</div>}>
              {truncate(x.genomic_dna_change, { length: 10 })}
            </Tooltip>
          ),
          filteredCases: (
            <span>
