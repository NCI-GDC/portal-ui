...
function getCaseLink({ cases: { hits: { total, edges: cases } }, file_id: fileId }: TNode): any {
  if (total > 1) {
    return (
    ...
    return <CaseLink id={cases[0].node.case_id}>{total}</CaseLink>;
  }

  return total;
}

const getAnnotationsLink = (annotations) => {
  if (annotations.total === 1) {
    ...
    return (<AnnotationsLink
      query={{
        filters: makeFilter([{ field: 'annotations.entity_id',
          value: annotations.edges.map(({ node: annotation }) => annotation.entity_id) }], false),
      }}
    >
      {annotations.total.toLocaleString()}
    </AnnotationsLink>);
  } else if (annotations.total > 20) {
    return (<Tooltip Component={<span>Link to the Annotation List page is temporarily unavailable</span>}>
      {annotations.total.toLocaleString()}
    </Tooltip>);
  }
...
export const FileTrComponent = ({ node, index, theme, canAddToCart = true, dispatch }: TProps) => (
  ...
  >
    ...
    <Td>{getAnnotationsLink(node.annotations.hits)}</Td>
  </Tr>
);
