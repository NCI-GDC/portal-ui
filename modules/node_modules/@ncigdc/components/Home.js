...
import { DATA_CATEGORIES, DATA_TYPES } from '@ncigdc/utils/constants';
import { fetchApi } from '@ncigdc/utils/ajax';

const transformChartData = data => {
  const hits = _.get(data, 'hits', false);
  ...
  const firstPassProjectData = _.filter(
    _.map(primarySiteIDs, (pID) => {
      let primarySiteData = primarySites[pID],
        caseCount = 0,
        fileCount = 0;
  ...
  .sort((primarySiteA, primarySiteB) => primarySiteB._count - primarySiteA._count);

  return _.map(firstPassProjectData, (primarySite) => {
    const dataStack : any = {};
      ...
      content: [
        {
          ...
          content: {
            ...
            value: [DATA_CATEGORIES.CNV.full],
          },
        },
        {
          op: 'in',
...
const Home = compose(
  ...
  lifecycle({
    async componentDidMount(): Promise<*> {
      ...
      const config = await fetchApi('status');
      const { data } = await fetchApi(
        'projects?facets=primary_site&fields=primary_site,project_id,summary.case_count,summary.file_count&filters=%7B%7D&from=1&size=1000&sort=summary.case_count:desc'
      );

      setState(state => ({ ...state, config, humanBodyLoading: false }));

      setTimeout(() => {
        const root = document.getElementById('human-body-root');
        sapien({
          // clickHandler: this.setFilters.bind(this),
          data: transformChartData(data).sort((a, b) => a._key > b._key ? 1 : -1),
