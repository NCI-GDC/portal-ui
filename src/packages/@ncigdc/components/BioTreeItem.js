...
const BioTreeItem = ({ entity, type, selectEntity, selectedEntity, query, expanded }) => (
    ...
    <div className="biospecimen-row-entity" style={{ marginBottom: '0.4rem' }}>
      ...
      <span
        className={`
              ...
              ${query && (search(query, { node: entity }) || []).map(e => e.node).some(e =>
            ...
            `}
        onClick={e => { selectEntity(entity, type); e.stopPropagation(); }}
      >
        <Highlight search={query}>{entity.submitter_id || entity[`${type.s}_id`]}</Highlight>
      </span>
