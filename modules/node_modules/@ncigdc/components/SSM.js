...
const Mutation = compose(
  ...
  lifecycle({
    async componentWillReceiveProps(nextProps) {
      if (!this.props.ssms.length && nextProps.ssms.length) {
        const transcript = nextProps.gene.transcripts.hits.edges.find(x =>
          x.node.is_canonical
        ).node;
  ...
  node: mutation,
  allCasesAgg,
  state: { gene, ProteinLolliplot, vizLoading },
}) => {
  const allCasesAggByProject = allCasesAgg.reduce((acc, bucket) =>
    ({ ...acc, [bucket.key]: bucket.doc_count }), {}
  );

  const externalDbIds = mutation.consequence.hits.edges.reduce((acc, c) => {
  ...
  const annotatedConsequence = mutation.consequence.hits.edges.find(c =>
    Object.keys(((c.node.transcript || {}).annotation || {})).length
  );

  const functionalImpact = ((annotatedConsequence || { node: { transcript: { annotation: { impact: '' } } } })
    .node.transcript.annotation.impact || '').toLowerCase();
  ...
  const consequenceData = mutation.consequence.hits.edges.reduce((acc, c) => {
    ...
    return {
      ...
      [gene_id]: {
        ...acc[gene_id],
        gene_id,
        gene_symbol: c.node.transcript.gene.symbol,
        transcripts,
        ...canonicalOnly,
      },
    };
