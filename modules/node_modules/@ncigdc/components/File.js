...
const File = ({
  ...
  const filteredAE = node.associated_entities.hits.edges
    ...
    .map(({ node: ae }) => {
      const annotations = node.annotations.hits.edges.filter(({ node: a }) => a.entity_id === ae.entity_id);
      // figure out how to link to a annotations page filtered on id
      return {
        ...ae,
        case_id: <CaseLink id={ae.case_id}>{ae.case_id}</CaseLink>,
        entity_id: <CaseLink id={ae.case_id}>{ae.entity_id}</CaseLink>, // TODO: link to Biospecimen section with search
        annotation_count: annotations.length ? <AnnotationsLink>{annotations.length}</AnnotationsLink> : '0',
      };
    });
      ...
      <Row style={{ alignItems: 'flex-start' }}>
        <EntityPageVerticalTable
          ...
          thToTd={[
            ...
            { th: 'Project ID', td: projectIds.map(pId => pId && <ProjectLink key={pId} id={pId}>{pId}</ProjectLink>) },
          ]}
        />
        <EntityPageVerticalTable
          ...
          <Row style={{ paddingTop: '2rem', alignItems: 'flex-start' }}>
            <EntityPageVerticalTable
              ...
              thToTd={[
                ...
                { th: 'Source Files', td: node.analysis.input_files.hits.total || '0' },
              ]}
            />
            <EntityPageVerticalTable
                ...
                data={
                  node.downstream_analyses.hits.edges.reduce((acc, { node: { output_files, workflow_type } }) => [
                    ...
                    ...output_files.hits.edges.map(({ node: file }) => ({
                      ...file,
                      file_name: <FileLink id={file.file_id}>{file.file_name}</FileLink>,
                      workflow_type,
                      action: (
                        <Row>
                          <AddToCartButtonSingle
