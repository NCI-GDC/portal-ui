...
import { addAllFilesInCart, fetchFilesAndAdd, fetchFilesAndRemove, toggleFilesInCart } from '@ncigdc/dux/cart';
...
type TProps = { edges: Array<TCartFile>, files: Array<TCartFile>, total: number, dispatch: Function };
  ...
  <LocationSubscriber>{ctx => {
    ...
    return (
      <Row>
        ...
        >
          <ShoppingCartIcon /><Hidden>{inCart ? 'Remove all files from cart' : 'Add all files to cart'}</Hidden>
        </CartButton>
        <Dropdown
        ...
        >
          ...
          >
            <ShoppingCartIcon style={{ marginRight: '1em' }} /> Add all files to the Cart
          </DropdownItem>
          <DropdownItem
            onClick={() => dispatch(fetchFilesAndRemove(currentFilters, total))}
            aria-label='Remove all files from cart'
            style={DropDownItemStyle}
          >
            <TrashIcon style={{ marginRight: '1em' }} /> Remove all from the Cart
          </DropdownItem>
        </Dropdown>
      </Row>
    );
