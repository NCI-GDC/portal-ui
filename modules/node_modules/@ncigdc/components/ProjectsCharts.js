...
function getGenes({ relay, caseCountFilters, projectIds }: TProps): void {
  relay.setVariables({
    fetchGeneData: true,
...
const ProjectsCharts = createRefetchContainer(
  ProjectsChartsComponent,
  {
    explore: graphql.experimental`
      fragment ProjectsCharts_explore on Explore
      @argumentDefinitions(
          fetchGeneData: {type: "Boolean", defaultValue: false}
          fmgCaseCount_filters: {type: "FiltersArgument"}
          fmgChart_filters: {type: "FiltersArgument"}
      ) {
        cases { hits(first: 0 filters: $fmgCaseCount_filters) { total } }
        genes {
          h: hits {
        ...
        genes @include(if: $fetchGeneData) {
          hits (first: 20 filters: $fmgChart_filters, score: "case.project.project_id") {
            total
            edges {
              node {
                score
    ...
    `,
    hits: graphql`
      fragment ProjectsCharts_hits on ProjectConnection {
        total
        edges @relay(plural: true) {
          node {
            id
  ...
  graphql.experimental`
    query ProjectsChartsRefetchQuery(
      $fetchGeneData: Boolean
      $fmgCaseCount_filters: FiltersArgument
      $fmgChart_filters: FiltersArgument
    ) {
      explore {
        ...ProjectsCharts_explore @arguments(
          fetchGeneData: $fetchGeneData
          fmgCaseCount_filters: $fmgCaseCount_filters
          fmgChart_filters: $fmgChart_filters
        )
      }
    }
  `
