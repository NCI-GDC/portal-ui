...
const AddCaseFilesToCartButton = compose(
  ...
  withHandlers({
    handleDropdownActivate: ({ relay, setIsLoading, filters, hasFiles }) => () => {
      if (!hasFiles) {
      ...
      hasFiles
        ...
        : (
          <Tooltip Component={<span>This case has no files.</span>}>
            <button className="btn disabled fa fa-shopping-cart" style={{ padding: '0px 17px 0 4px' }} aria-label="This case has no files" />
          </Tooltip>
        )
    }
