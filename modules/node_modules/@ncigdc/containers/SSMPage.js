...
const CancerDistributionTitle = ({ cases = 0, projects = 0 }: TChartTitleProps) => (
  <h5 style={{ textTransform: 'uppercase', padding: '0 2rem' }}>
    THIS MUTATION AFFECTS {cases.toLocaleString()} CASES ACROSS {projects.toLocaleString()} PROJECTS
  </h5>
);
...
export const SSMPageComponent = compose(
  withPropsOnChange(['node'], ({ node }) => ({
    canonicalGeneId: node.consequence.hits.edges[0].node.transcript.gene.gene_id,
...
export const SSMPageQuery = {
  initialVariables: {
    isCanonical: makeFilter([
      { field: 'consequence.transcript.is_canonical', value: [true] },
    ], false),
  },
  fragments: {
    node: () => Relay.QL`
