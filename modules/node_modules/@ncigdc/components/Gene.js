...
const Gene = compose(
  ...
  lifecycle({
    async componentDidMount(): Promise<*> {
      ...
      const transcript = gene.transcripts.hits.edges.find(x => x.node.is_canonical).node;
      ...
      this.props.setState(s => ({ ...s, gene, mutations: data.hits }));
    },
  })
)
(({ state: { gene, mutations, ProteinLolliplot }, ...props }) => {
  ...
  const cancerDistData = props.node.case.hits.edges.map(x => x.node).reduce((acc, c) => {
    ...
    return {
      ...
      [c.project.project_id]: {
        ...
        site: c.primary_site,
        cases,
        ssms: [...new Set([...(acc[c.project.project_id] || { ssms: [] }).ssms, ...c.ssm.hits.edges.map(m => m.node.ssm_id)])],
      ...
      <Row spacing="2rem">
        <EntityPageVerticalTable
          ...
          thToTd={[
            ...
            {
              th: 'Location',
              td: `chr${gene.gene_chromosome}:${gene.gene_start}-${gene.gene_end} (${(((props.node.case.hits.edges[0] || {}).ssm || [])[0] || {}).ncbi_build})`,
            },
