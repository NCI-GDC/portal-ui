...
import { compose, withState, withProps, lifecycle, mapProps, withPropsOnChange } from 'recompose';
...
const ProjectsChartsComponent = compose(
  ...
  const primarySiteToColor = Object.keys(primarySiteProjects).reduce((primarySiteAcc, primarySite) => ({
    ...primarySiteAcc,
    [primarySite]: {
      ...primarySiteProjects[primarySite],
      projects: primarySiteProjects[primarySite].projects.reduce((acc, projectId, i) => ({
        ...acc,
        [projectId]: d3.color(primarySiteProjects[primarySite].color)
          ...
          <div style={{ alignSelf: 'center', color: theme.greyScale7, fontSize: '1.2rem' }} key='bar-subtitle'>
            <ExploreLink
              query={{
                ...
                filters: caseCountFilters ? { op: 'and', content: caseCountFilters } : null,
              }}
            >{numUniqueCases.toLocaleString()}</ExploreLink>
            {
