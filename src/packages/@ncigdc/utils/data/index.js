...
export const buildProteinLolliplotData = ({
  ...
  mutations: data
    .filter(x => {
      const co = x.consequence.find(c => c.transcript.transcript_id === transcript.transcript_id);
      return co && co.transcript.aa_start;
    })
    .map(mutation => {
      // need to filter here on client unless filtered on server
      const consequence = mutation.consequence.find(c => c.transcript.transcript_id === transcript.transcript_id);
      ...
      return {
        ...
        consequence: consequence.transcript.consequence_type.replace('_variant', ''),
