...
const InternalLinkWithContext = ({ pathname, query, merge, mergeQuery, whitelist, ...rest }: TLinkProps) => (
  <LocationSubscriber>{
    (ctx: {| pathname: string, query: TRawQuery |}) => {
      ...
      const queryWithOffsetsReset = (
        hasFilterChanged
          ? mergedQuery
          : _.mapValues(mergedQuery,
