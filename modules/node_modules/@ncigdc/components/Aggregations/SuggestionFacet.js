...
const StyledDropdownLink = styled(Link, ({
  ...
  backgroundColor: ({ isActive }) => (isActive ? 'rgb(31, 72, 108)' : 'inherit'),
  width: '100%',
  textDecoration: 'none',
}));
              ...
              <Row>
                ...
                <StyledInput
                  ...
                  onChange={(e) => {
                    ...
                    if (!!value) {
                      setIsLoading(true);
                      setAutocomplete(value, (readyState) => _.some([readyState.ready, readyState.aborted, readyState.error]) && setIsLoading(false));
                    }
                  }}
                  onKeyDown={selectableList.handleKeyEvent}
                  placeholder={placeholder}
                  value={inputValue}
                  aria-activedescendant={_.get(selectableList, 'focusedItem.id')}
                  {...active && { 'aria-owns': `${fieldNoDoctype}-options` }}
                />
                {active &&
                <Column
