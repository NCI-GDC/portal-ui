...
const CancerDistributionTableComponent = compose(
  ...
  withPropsOnChange(
    ...
    ({ cases, projects, geneId, entityName, explore }: TProps) => {
      const casesByProjectMap = get(cases.total, 'project__project_id.buckets', [])
        .reduce((acc, bucket) => ({ ...acc, [bucket.key]: bucket.doc_count }), {});
      const projectsById = groupBy((projects.hits || { edges: [] }).edges, e => e.node.project_id);

      // eslint-disable-next-line fp/no-mutating-methods
      const rawData = (cases.filtered || { project__project_id: { buckets: [] } })
        .project__project_id.buckets.map(b => {
          ...
          return {
            project_id: b.key,
            disease_type: project ? (project.node.disease_type || []).join(', ') : null,
        ...
        .sort((a, b) => b.num_affected_cases_percent - a.num_affected_cases_percent);
      ...
      const cancerDistData = rawData.map((row) => {
        const projectFilter = makeFilter(
          [baseFilter, { field: 'cases.project.project_id', value: [row.project_id] }],
          false
        );

        return {
          freq: row.num_affected_cases_percent,
          project_id: <ProjectLink uuid={row.project_id}>{row.project_id}</ProjectLink>,
          disease_type: row.disease_type || <GreyBox />,
          site: row.site || <GreyBox />,
          num_affected_cases: (
            <span>
              <ExploreLink query={{ searchTableTab: 'cases', filters: projectFilter }}>
              ...
              <span>&nbsp;({(row.num_affected_cases_percent * 100).toFixed(2)}%)</span>
            </span>
          ),
  ...
  withPropsOnChange(
    ...
    ({ cancerDistData, query }) => {
      ...
      return {
        ...
        enablePagination,
        tableData: enablePagination ? cancerDistData.slice(offset, offset + size) : cancerDistData,
      };
    }
  )
)(({
  ...
  return (
    ...
    >
      <Row
        style={{ backgroundColor: 'white', padding: '1rem', justifyContent: 'space-between', alignItems: 'flex-end' }}
      >
        {enablePagination ? (
          <Showing
            docType="projects"
        ...
        <Row style={{ alignItems: 'flex-end' }}>
          <ExploreLink query={{ searchTableTab: 'cases', filters }}>Open in Exploration</ExploreLink>
          <Tooltip Component={<span>Export Table</span>} style={{ marginLeft: '2rem' }}>
            <Button
              style={{ ...visualizingButton }}
              onClick={() => saveFile(JSON.stringify(rawData, null, 2), 'JSON', 'cancer-distribution-data')}
            >
              <DownloadIcon />
            </Button>
          </Tooltip>
