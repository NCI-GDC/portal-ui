...
export type TChartTitleProps = { cases: number, projects: Array<{ project_id: string }>, ssms: number, filters: any };
const DefaultChartTitle = ({ cases = 0, projects = [], ssms = 0, filters }: TChartTitleProps) => (
  <h5 style={{ textTransform: 'uppercase', padding: '0 2rem' }}>
    ...
    cases affected by&nbsp;
    <ExploreLink query={{ searchTableTab: 'mutations', filters }}>{ssms.toLocaleString()}</ExploreLink>&nbsp;
    mutations across&nbsp;
    <ProjectsLink
      query={{
        filters: {
  ...
  const chartData = sortedCancerDistData.map(d => ({
    ...
    tooltip: (
      <span>
        ...
        Case{d.num_affected_cases > 1 ? 's' : ''} Affected in <b>{d.project_id}</b><br />
        {d.num_affected_cases.toLocaleString()} / {d.num_cases_total.toLocaleString()}&nbsp;
    ...
    <span>{
      ...
      >
        <Column>
          ...
          <Row style={{ padding: '0 2rem', justifyContent: 'space-between' }}>
            <BarChart
