...
class GitHut extends React.Component {
  componentDidMount() {
    ...
    angular
      ...
      .directive('githut', ['$timeout', ($timeout) => ({
        ...
        link(scope, element, attrs, controller) {
          ...
          const hideDuplicates = () => {
            const duplicatePrimarySiteTexts = _.flatten(Object.values(_.groupBy(element.find('.githut #pc svg #labels .label:last-child text'), 'innerHTML')).filter(els => els.length > 1).map(_.tail));
