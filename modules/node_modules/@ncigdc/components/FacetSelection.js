...
import { compose, withState, lifecycle, withProps, renameProps, withPropsOnChange, defaultProps, withHandlers, getContext } from 'recompose';
...
export default compose(
  ...
  withHandlers({
    fetchData: ({ setFacetMapping, setIsLoading }) => async () => {
      setIsLoading(true);
  ...
  withProps(({ facetMapping, excludeFacetsBy, query }) => ({
    filteredFacets: _.filter(_.values(facetMapping), (facet) => facetMatchesQuery(facet, query) && !excludeFacetsBy(facet)),
  })),
  renameProps({
    onSelect: 'handleSelectFacet',
  }),
  withHandlers({
    handleClose: ({ setQuery, setFocusedFacet, onRequestClose }) => () => {
      setQuery('');
  ...
  <div>
    <div {...css(styles.header)}>
      ...
      <label tabIndex={0} role="button" className="pull-right" style={{ display: 'none' }}>
        <input type="checkbox" />
