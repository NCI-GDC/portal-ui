...
import { groupBy, isEqual, head } from 'lodash';
...
const CancerDistributionTableComponent = compose(
  ...
  const rawData = (cases.filtered || { project__project_id: { buckets: [] } })
    .project__project_id.buckets.map(b => {
      ...
      return {
        ...
        num_affected_cases_percent: (freq * 100).toFixed(2),
      };
    })
    .sort(d => -d.freq);
    ...
    return {
      ...
      num_affected_cases: (
        <span>
          ...
          <span>&nbsp;({row.num_affected_cases_percent}%)</span>
        </span>
      ),
  ...
  return (
    <Loader
      ...
      height="387px"
    >
      <Row style={{ backgroundColor: 'white', padding: '1rem', justifyContent: 'flex-end', alignItems: 'flex-end' }}>
