  ...
  lifecycle({
    async componentDidMount(): Promise<*> {
      ...
      const { data } = await fetchApi(
        `analysis/top_mutated_high_impact_genes?project_ids=${projectIds.join()}&fields=gene_id,symbol`
      );

      const topGenesSource = data.hits || [];

      const { aggregations } = await fetchApi(
        `analysis/top_cases_counts_by_genes?gene_ids=${topGenesSource.map(g => g.gene_id).join()}`
      );
