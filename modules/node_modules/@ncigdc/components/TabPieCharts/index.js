...
const toPieData = (clickHandler, docTypeSingular ) => bucket => ({
  ...
  tooltip: (
    <span>
      <b>{bucket.key}</b><br />
      {bucket.doc_count.toLocaleString()} {docTypeSingular}{bucket.doc_count > 1 ? 's' : 0}
    </span>
  ),
});
...
export const SelfFilteringPie = ({ docTypeSingular, buckets, query, push, fieldName, currentFieldNames, currentFilters, ...props }) => (
  <PieChart
    data={(buckets || [])
      .filter(bucket => bucket.key !== '_missing')
      .filter(bucket => (currentFieldNames.includes(fieldName) ?
                         inCurrentFilters({ key: bucket.key, dotField: fieldName, currentFilters }) : true))
      .map(toPieData(({ data }) => addFilter(query, push)(fieldName, data.id), docTypeSingular))
    }
    {...props}
  />
);
