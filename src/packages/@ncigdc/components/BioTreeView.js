...
const BioTreeView = ({ entities, type, expanded: ex, setExpanded, selectEntity, selectedEntity, query, childrenExpanded }) => {
  const expanded = ex || (query && entities.hits.edges.some(e => search(query, e).length));
  return (
    <div>
      {entities.hits.total > 0 &&
        <div onClick={e => { setExpanded(state => !state); e.stopPropagation(); }} className="tree-indent">
          <div className="tree">
            <i
...
const enhance = compose(
  withState('expanded', 'setExpanded', props => props.defaultExpanded || props.entities.expanded),
  withState('emitterToken', 'setEmitterToken', null),
  mapProps(({ defaultExpanded, ...rest }) => ({ childrenExpanded: defaultExpanded, ...rest })),
  lifecycle({
