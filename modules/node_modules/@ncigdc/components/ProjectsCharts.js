...
import { setFilter, mergeQuery, makeFilter } from '@ncigdc/utils/filters';
import { removeEmptyKeys } from '@ncigdc/utils/uri';
import convertProjectFilterToCaseFilter from '@ncigdc/utils/filters/convertProjectFilterToCaseFilter';
...
function getGenes({ relay, filters }: TProps): void {
  const massagedFilters = filters && traverse(filters).map(function () {
    if (this.key === 'field') this.update(convertProjectFilterToCaseFilter(this.node))
  });

  relay.setVariables({
    fetchGeneData: true,
    fmgCaseCount_filters: massagedFilters,
    fmgChart_filters: {
      op: 'AND',
      content: [
        ...massagedFilters,
