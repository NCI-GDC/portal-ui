...
const ResultIcon = ({ type, style }) => (<span style={style}>{entityShortnameMapping[type] || type}</span>);
...
export default ({ results = [], query, isLoading, onSelectItem, onActivateItem, style = {} }: TProps) => (
      ...
      results.map((item, i) => (
        ...
        >
          <div style={styles.itemIconWrapper}>
            <ResultIcon type={atob(item.id).split(':')[0]} style={styles.itemIcon} />
          </div>
          <div>
            <span style={styles.itemTitle}>{ item.symbol || atob(item.id).split(':')[1] }</span>
