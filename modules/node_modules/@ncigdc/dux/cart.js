...
const messageNotificationDispatcher = (action: string, message, dispatch: Function) => {
  dispatch(notify({
    action,
    id: `${(new Date()).getTime()}`,
    component: (
...
function toggleFilesInCart(incomingFile: TCartFile | Array<TCartFile>): Function {
  return (dispatch, getState) => {
    const incomingFileArray = Array.isArray(incomingFile) ? incomingFile : [incomingFile];
...
function addAllFilesInCart(incomingFiles: Array<TCartFile> | TCartFile): Function {
  return (dispatch, getState) => {
    const incomingFilesArray = Array.isArray(incomingFiles) ? incomingFiles : [incomingFiles];
    const existingFiles = getState().cart.files;
...
function fetchFilesAndAdd(currentFilters: ?Object, total: number): Function {
  return async (dispatch) => {
    ...
    if (total <= MAX_CART_SIZE) {
      messageNotificationDispatcher('info',
...
function fetchFilesAndRemove(currentFilters: ?Object, size: number): Function {
  return async (dispatch, getState) => {
    ...
    if (!existingFiles.length) {
      dispatch(notify({
        action: 'warning',
        component: <Column><span>There are no files in the cart to remove.</span></Column>,
      }));
    ...
    const filters = size > MAX_CART_SIZE ? replaceFilters({
      op: 'and',
      content: [
        { op: 'in', content: { field: 'files.file_id', value: existingFiles.map(f => f.file_id) } },
      ],
