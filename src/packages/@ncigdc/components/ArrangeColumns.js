...
const ArrangeColumns = compose(
  ...
  withState('state', 'setState',
    ({ entityType }) => ({ draggingIndex: null, columns: tableModels[entityType] })
  ),
  pure
)(({
  dispatch,
            ...
            if (!nextState.items && state.items) {
              const nextColumnIds = state.items.reduce((acc, x) => ([
                ...
                ...(tableColumns.includes(x.id)
                  ? [x.id, ...tableModels[entityType].filter(c => c.parent === x.id).map(c => c.id)]
                  : []
                ),
              ]), []);
            ...
            <Row
              ...
              onClick={
                () => {
                  ...
                  const subHeadingCol = tableModels[entityType].find(x => x.subHeadingIds);
                  // find current index of subheading columm
                  const subHeadingColIndex = tableColumns.indexOf(subHeadingCol.id);
                  // figure out whether to put before or after column with subheadings
                  const afterSubheadingCol = subHeadingColIndex !== -1 && subHeadingColIndex < i;

                  dispatch(toggleColumn({
                    entityType,
                    id: column.id,
                    // if after subheading col include number of subheadings to place inbetween
                    index: afterSubheadingCol && !column.subHeadingIds ? i + subHeadingCol.subHeadingIds.length : i,
                  }));
                }
              }
            >
