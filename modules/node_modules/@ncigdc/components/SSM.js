...
const Mutation = compose(
  ...
  lifecycle({
    async componentDidMount(): Promise<*> {
      const { node, renderProteinLolliplot, setState } = this.props;
      const canonicalTranscript = (node.consequence.find(x => x.transcript.is_canonical) || {}).transcript || {};
      const { transcript_id: tid, gene: { gene_id: geneId } = {} } = canonicalTranscript;

      const results = await Promise.all([
        tid && fetchApi('analysis/frequent_mutations_by_project', {
          method: 'POST',
      ...
      const data = (results[0] || { data: {} }).data.hits;
      ...
      if (gene && data) {
        ...
        renderProteinLolliplot(proteinLolliplotData, transcript, this.props);
      }

      setState(s => ({
        ...s,
        gene: gene || initialState.gene,
      }));
    },
  ...
  const cancerDistData = mutation.occurrence.reduce((acc, o) => {
    const cases = uniq([
      ...(acc[o.case.project.project_id] || { cases: [] }).cases,
      o.case.case_id,
    ]);
    return {
      ...acc,
      [o.case.project.project_id]: {
        disease_type: o.case.disease_type,
        cancer_type: o.case.project.cancer_type || 'tbd',
        site: o.case.primary_site,
  ...
  const consequenceData = mutation.consequence.reduce((acc, c) => {
    const { gene: { gene_id }, transcript_id } = c.transcript;
    ...
    return {
      ...
      [gene_id]: {
        ...acc[gene_id],
        gene_id,
        gene_symbol: c.transcript.gene.symbol,
        transcripts,
        ...canonicalOnly,
      },
    };
  ...
  const consquenceDataMapped = Object.values(consequenceData)
    .map(d => ({
      ...d,
      gene_symbol: <GeneLink id={d.gene_symbol}>{d.gene_symbol}</GeneLink>,
      transcripts: (
