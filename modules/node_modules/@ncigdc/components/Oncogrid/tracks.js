...
function getSquare(track: Object): string {
  const opacityStyle = typeof track.opacity === 'number' ? `; opacity: ${track.opacity}` : '';
  return `<div class="onco-track-legend" style="background: ${getColorValue(track)}${opacityStyle}"></div>`;
}
...
function legendMulti(track): string {
  const joinCharacter = track.joinCharacter || '<br>&nbsp;&nbsp;';
...
export const clinicalDonorTracks = [
  ...
  },
  {
    ...
    group: 'Clinical',
    color: '#638f56',
    template: '{{displayId}}<br>{{displayName}}: {{displayValue}}{{#notNullSentinel}} days{{/notNullSentinel}}',
