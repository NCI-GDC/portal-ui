...
const buildProteinLolliplotData = ({ transcript, data }) => ({
  mutations: data
    .filter(mutation => mutation.consequence.find(c => c.transcript.transcript_id === transcript.id))
    .map(mutation => {
      const consequence = mutation.consequence.find(c => c.transcript.transcript_id === transcript.id);
      return {
        id: mutation.ssm_id,
