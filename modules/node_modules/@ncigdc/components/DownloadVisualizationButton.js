  ...
  <DropDown
    button={
      <Tooltip Component={tooltipHTML}>
        ...
        >
          {noText ? <span><Download /><Hidden>Download</Hidden></span> : 'Download'}
        </Button>
      </Tooltip>
    }
    {...props}
      ...
      tsvData && (
        <DropdownItem
          ...
          onClick={() => {
            if (tsvData) {
              saveFile(
                tsvData[0] && tsvData[0].forEach ? mapArrayToTsvString(tsvData) : toTsvString(tsvData),
                'TSV',
                `${slug}.tsv`
              );
            }
