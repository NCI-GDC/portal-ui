...
export const toggleFilters: TMergeFilters = (q, ctxq) => {
  ...
  const merged = {
    ...
    content: ctxq.content.reduce((acc, ctx) => {
      const found = acc.find(a => a.content.field === ctx.content.field && a.op === ctx.op);
...
export const getFilterValue = ({ currentFilters, dotField }) => currentFilters.find(f => f.content.field === dotField);

type TMakeFilter = (fields: [{ field: string; value: string }], returnString: boolean) => Object | string;
