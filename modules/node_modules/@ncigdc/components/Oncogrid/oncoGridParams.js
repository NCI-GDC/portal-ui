...
function dataTypeLegend(): string {
  return `<div><b>Available Data Types:</b></div><div>${
    dataTypeTracks
...
function gdcLegend(max: number): string {
  return `<div><b># of Cases Affected:</b></div><div>${
    gdcTracks
...
function geneSetLegend(): string {
  return `<div><b>Gene Sets:</b></div><div>${
    geneSetTracks
      .map(t => t.legend ? t.legend(t) : '')
      .filter(Boolean)
      .join('</div><div>')
...
export default function ({
  ...
  const {
    ...
    donorIds,
    geneIds,
  } = buildOccurences(occurencesData, donorData, geneData, consequenceTypes, impacts);
  ...
  return {
    ...
    trackHeight: 12,
    trackLegends: {
      Clinical: `<div><b>Clinical Data:</b></div><div>${
        clinicalDonorTracks
