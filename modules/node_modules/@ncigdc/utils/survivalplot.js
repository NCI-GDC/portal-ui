...
export const getDefaultCurve = async ({ slug, totalCases, currentFilters }: TPropsDefault): Promise<Object> => {
  ...
  const legend = hasEnoughData ?
    totalCases && slug && [{ key: slug, value: `${totalCases} Cases in ${slug}` }] :
...
export const getSurvivalCurves = async ({ value, field, slug, currentFilters, shortMutationId }: TPropsMulti): Promise<Object> => {
  const filters = [
    replaceFilters(
    ...
    legend: hasEnoughData ? [
      {
        ...
        value: (
          <span>
            S<sub>1</sub>(N={rawData.results[0].donors.length}) <Symbol>{shortMutationId ? value.substr(0, 8) : (slug || value)}</Symbol> Mutated Cases
          </span>
        ),
      },
      {
        ...
        value: (
          <span>
            S<sub>2</sub>(N={rawData.results[1].donors.length}) <Symbol>{shortMutationId ? value.substr(0, 8) : (slug || value)}</Symbol> Not Mutated Cases
          </span>
        ),
      },
    ] : [
