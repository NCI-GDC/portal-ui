      ...
      consequence_type: (
        <span>
          ...
          <span style={{ marginLeft: '5px' }}>
            {geneId && symbol && <GeneLink id={geneId}>{symbol}</GeneLink>}
          </span>
          <span
            style={{
              marginLeft: '5px',
...
const mapRelayData = (data: Array<Object>, projectId: string): Array<Object> => (
  data.map((hit) => {
...
compose(
  ...
  withProps({
    fetchBarChart: async ({ currentFilters, projectId, setState }) => {
      ...
      const filters = currentFilters && currentFilters.content.length ? currentFilters : null;
  ...
  return !!state.frequentMutationsTable.length && (
    ...
    >
      <FrequentMutations
        ...
        frequentMutationsChart={props.ssms
          ? mapRelayData(props.ssms.map(x => x.node), props.projectId)
          : state.frequentMutationsChart
        }
