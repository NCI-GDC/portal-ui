...
const ClinicalVariableCard: React.ComponentType<IVariableCardProps> = ({
  ...
  const getCategoricalSetFilters = () => {
    console.log('dataBu', variable.bins, selectedBuckets);
    // (selectedBuckets|| []).map(bucket => bucket)
    const bucketFilters = []
  ...
  const cardFilters = get(selectedBuckets, 'length', 0)
...
export default compose(
  ...
  withProps(({ dataBuckets, variable }) => ({
    customBins: Object.keys(variable.bins).length > 0
      ...
      : dataBuckets.map(b => ({
        key: b.key,
        doc_count: b.doc_count, // TODO:  Remove this
        groupName: b.key,
      })),
  })),
  withProps(
