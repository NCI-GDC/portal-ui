...
const BiospecimenCard = ({ p, state: { selectedEntity: se, type, query }, setState, theme }) => {
  ...
  return (
    <Card title="Biospecimen" style={{ flex: 1 }}>
      <Row>
          ...
          <EntityPageVerticalTable
            thToTd={[
              ...
              ...Object.entries(selectedEntity)
                .filter(([key]) =>
                  key !== 'submitter_id' && key !== 'expanded' && key !== `${type}_id` && key !== '__dataID__'
                )
                .map(([key, val]) => {
                  if (key === 'portions') {
                    return { th: humanify({ term: key }), td: formatValue(val.hits.edges.length) };
                  }
