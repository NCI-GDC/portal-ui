...
const LolliplotComponent = compose(
  ...
  withProps(p => ({
    ...
    fetchSsms(props: Object): void {
      const transcript = (props.viewer.node.transcripts.hits.edges.find(x => x.node.is_canonical) || {}).node;
  ...
  lifecycle({
    ...
    async componentWillReceiveProps(nextProps: TProps): Promise<*> {
      const { ssms, state, renderProteinLolliplot, setState } = this.props; // eslint-disable-line fp/no-this

      if (this.props.relay.route.params.id !== nextProps.relay.route.params.id) {
