...
export const mapDonors = (donors: Array<TDonorInput>): Array<TDonor> => (
  donors.map(({ summary, demographic = {}, case_id, diagnoses = [] }): TDonor => {
    ...
    summary.data_categories.forEach((category) => {
      output[category.data_category] = category.file_count || 0; // eslint-disable-line fp/no-mutation
    });
...
function expandObs(geneIdToSymbol: {}): Function {
  return ({ ssm, case: casObj = {} }): Array<TOccurence> => ssm.consequence
    .map(c => c.transcript)
    .filter(({ annotation = {} }) => annotation.impact)
...
export const mapOccurences: TMapOccurences = (occurrences, donors, genes, consequenceTypes, impacts) => {
