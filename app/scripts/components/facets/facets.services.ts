module ngApp.components.facets.services {
  ...
  class CustomFacetsService implements ICustomFacetsService {
    ...
    private dsHasValues: restangular.IElement;
    ...
    constructor(private Restangular: restangular.IService,
                ...
                this.dsHasValues = Restangular.all("ui/has-facets-with-values");
    }
    ...
    getFacetFields(docType: string): ng.IPromise<any> {
      return this.ds.getList().then((data) => this.filterFields(docType, data));
    }

    getNonEmptyFacetFields(docType: string, fields: Array<Object>): ng.IPromise<any> {
      var options = { filters: this.LocationService.filters() };
      return this.dsHasValues.get(docType, options)
                 .then(data => this.filterFields(docType, _.filter(fields, f => _.includes(data.data || [], f.field))));
    }
  }
