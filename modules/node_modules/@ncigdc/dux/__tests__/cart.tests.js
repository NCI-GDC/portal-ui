...
describe('action creators', () => {
  ...
  it('should allow up to MAX_CART_SIZE files to be added', () => {
    const getState = () => ({
      cart: { files: range(0, MAX_CART_SIZE - 1).map(id => ({ ...fileA, file_id: id })) },
    });
    const dispatch = jest.fn();
    addAllFilesInCart(fileA)(dispatch, getState);
    expect(dispatch).toHaveBeenCalledTimes(2);
    expect(dispatch).toHaveBeenCalledWith({ type: ADD_TO_CART, payload: [fileA] });
  });

  it('should not allow more than MAX_CART_SIZE files to be added', () => {
    const getState = () => ({
      cart: { files: range(0, MAX_CART_SIZE).map(id => ({ ...fileA, file_id: id })) },
    });
    const dispatch = jest.fn();
    addAllFilesInCart(fileA)(dispatch, getState);
    expect(dispatch).toHaveBeenCalledTimes(2);
  ...
  it('should allow a file to be toggled in', () => {
    ...
    expect(dispatch).toHaveBeenCalledWith({ type: UPDATE_CART, payload: [fileA] });
  });
  ...
  it('should allow up to max to be toggled in', () => {
    ...
    const dispatch = jest.fn();
    toggleFilesInCart(fileA)(dispatch, getState);
    expect(dispatch).toHaveBeenCalledWith({ type: UPDATE_CART, payload: [...getState().cart.files, fileA] });
  });
