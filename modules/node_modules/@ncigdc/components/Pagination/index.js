...
export const getPaginationRange = (pageOffset, totalPages) => {
  ...
  return _.range(1 + pageOffset, Math.min(numPagesToShow + pageOffset, totalPages) + 1);
};

const Pagination = (props: TProps) => {
  const { prfOff, prfSize, size, prev,
          last, next, prevPred, nextPred,
          currentPage, totalPages, pageOffset } = calculatePages(props);

  return (
    <Row style={styles.topRow(props.theme)}>
      <Row style={{ alignItems: 'center' }}>
        <span style={{ marginRight: '1rem' }}>Show</span>
        <Sizes prfSize={prfSize} prfOff={prfOff} size={size} sizes={props.sizes} />
      ...
      <Row style={{ marginLeft: 'auto' }}>
        <PaginationLink pred={prevPred} prfOff={prfOff} offset={0}>
          <PaginationBtn
            style={styles.leftBtn(props.theme)}
          >
            {'«'}
          </PaginationBtn>
        </PaginationLink>
        ...
        <PaginationLink pred={nextPred} prfOff={prfOff} offset={next}>
          <PaginationBtn style={styles.middleBtn(props.theme)}>{'›'}</PaginationBtn>
        </PaginationLink>
        <PaginationLink pred={nextPred} prfOff={prfOff} offset={last}>
          <PaginationBtn style={styles.rightBtn(props.theme)}>{'»'}</PaginationBtn>
        </PaginationLink>
      </Row>
    </Row>
  );
