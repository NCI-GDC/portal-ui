      ...
      <table style={styles.table}>
        <thead>
          <Tr>
            ...
            <Th colSpan={Object.entries(DATA_CATEGORIES).length}>Available Cases per Data Category</Th>
            <Th rowSpan="2">Files</Th>
          </Tr>
          <Tr>
            {
              Object.values(DATA_CATEGORIES).map(category => (
                <Th key={category.abbr}>
                  <abbr>
        ...
        <tbody>
          ...
          <Tr style={{ borderTop: `1px solid ${props.theme.greyScale4}` }}>
            ...
            <Td>
              {
                props.totalCases > 0 ? (
                  <RepositoryCasesLink
                    query={{ filters: props.query.filters ? getProjectIdFilter(props.projects) : null }}
                  >
            ...
            {Object.keys(CATEGORY_MAP)
              ...
              .map((category) => {
                const count = sum(
                  props.projects.edges.map(p => findDataCategory(category, p.node.summary.data_categories).case_count)
                );
                            ...
                            filters: makeFilter([
                              {
                                ...
                                value: props.projects.edges.map(({ node: p }) => p.project_id),
                              },
                              { field: 'files.data_category', value: CATEGORY_MAP[category] },
                            ], false),
                          }}
                        >
            ...
            <Td>
              {
                props.totalFiles > 0 ? (
                  <RepositoryFilesLink
                    query={{
                      filters: props.query.filters ? getProjectIdFilter(props.projects) : null,
                    }}
                  >
