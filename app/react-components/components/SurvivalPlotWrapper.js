...
SurvivalPlotWrapper.defaultProps = {
  ...
  onMouseEnterDonor: (e, donor) => {
    ...
    `;  // can remove || when rebase with survival fetching update.
  },
  onMouseLeaveDonor: () => {
    const tooltip = document.querySelector('.global-tooltip')
  ...
  onClickDonor: (e, donor) => {
    window.location = `/cases/${donor.id || donor.meta.id}`; // can remove || when rebase with survival fetching update.
  },
};
