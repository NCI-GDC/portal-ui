...
const createRenderer = (Route, Container) => compose(
  connect(),
  withRouter
)((props: mixed) => (
  <Relay.Renderer
...
type TMapData = (data: Array<Object>, shouldShowGeneSymbol: boolean, theme: Object) => Array<Object>;
...
const mapData: TMapData = (data, shouldShowGeneSymbol, theme) => (
  data.map(hit => {
    ...
    const { annotation = {}, consequence_type: consequenceType = '', gene = {}, aa_change: aaChange } =
      transcript || {};
      ...
      consequence_type: (
        <span>
          ...
          {shouldShowGeneSymbol &&
            <GeneLink uuid={geneId}>{symbol}</GeneLink>
          }
          <Tooltip
            Component={<div style={{ maxWidth: 300, wordBreak: 'break-all' }}>{aaChange}</div>}
            style={{
...
const Component = compose(
  ...
  const frequentMutations = mapData(
    ssms ? orderBy(ssms.hits.edges.map(x => x.node), ['score', 'ssm_id'], ['desc', 'asc']) : [],
    shouldShowGeneSymbol,
    theme
  );
  ...
  return (
    <Loader
      ...
      height="387px"
    >
      <Row style={{ backgroundColor: 'white', padding: '1rem', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <Showing
          docType="mutations"
          prefix="ssmsTable"
      ...
      <EntityPageHorizontalTable
        ...
        data={frequentMutations.map(({ score = 0, ...x }) => ({ // eslint-disable-line
          ...x,
          mutation_uuid: (
            <span>
              <MutationLink uuid={x.ssm_id}>{x.ssm_id.substr(0, 8)}</MutationLink>
              <ForTsvExport>
                {x.ssm_id}
              </ForTsvExport>
            </span>
          ...
          genomic_dna_change: (
            <Tooltip Component={<div style={{ maxWidth: 300, wordBreak: 'break-all' }}>{x.genomic_dna_change}</div>}>
          ...
          filteredCases: (
            <span>
              ...
              <span>&nbsp;({((score / filteredCases.hits.total) * 100).toFixed(2)}%)</span>
            </span>
          ),
          projectBreakdown: (
            <ProjectBreakdown
