...
const Home = compose(
  ...
  lifecycle({
    async componentDidMount(): Promise<*> {
      const { setState, dispatch, push } = this.props; // eslint-disable-line

      setState(state => ({ ...state, humanBodyLoading: !this.props.humanBodyData.length }));

      const data = transformChartData({ hits: this.props.humanBodyData })
        .sort((a, b) => a._key > b._key ? 1 : -1);

      if (this.props.humanBodyData.length) {
        setTimeout(() => {
              ...
              dispatch(setTooltip(
                <span>
                  ...
                  <div style={{ fontSize: '12px', color: 'rgb(20, 20, 20)' }}>
                    {d._count} cases ({d.fileCount} files)
                  </div>
                </span>
              ));
            },
            mouseOutHandler: () => dispatch(setTooltip(null)),
            data,
