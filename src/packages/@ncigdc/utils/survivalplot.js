...
type TPropsMulti = { value: string, field: string, slug?: string, currentFilters?: Object, size?: number };
...
async function fetchCurves(filters: ?Array<Object>, size: number): Promise<Object> {
  const params = _.omitBy({ filters: filters && JSON.stringify(filters), size }, _.isNil);
...
export const getDefaultCurve = memoize(async ({
  slug,
  currentFilters,
  size,
  ...
  const legend = hasEnoughData ?
    slug && [{ key: slug, value: `${rawData.results[0].donors.length.toLocaleString()} Cases with Survival Data` }] :
...
export const getSurvivalCurves = memoize(async ({
  value,
  field,
  slug,
  currentFilters,
    ...
    legend: hasEnoughData ? [
      {
        ...
        value: (
          <span>
            S<sub>1</sub> (N = {rawData.results[0].donors.length.toLocaleString()}) - <Symbol>{slug || value}</Symbol> Not Mutated Cases
          </span>
        ),
      },
      {
        ...
        value: (
          <span>
            S<sub>2</sub> (N = {rawData.results[1].donors.length.toLocaleString()}) - <Symbol>{slug || value}</Symbol> Mutated Cases
          </span>
        ),
      },
    ] : [
