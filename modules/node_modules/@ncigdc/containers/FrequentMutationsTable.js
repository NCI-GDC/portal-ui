    ...
    <div>
      ...
      <EntityPageHorizontalTable
        ...
        data={frequentMutations.map(x => ({
          ...x,
          genomic_dna_change: <MutationLink id={x.ssm_id}>{x.genomic_dna_change}</MutationLink>,
          ...
          num_affected_cases_all: (
            <TogglableUl
              items={[
                ...
                <span>
                  {x.num_affected_cases_all} ({((x.num_affected_cases_all / cases.hits.total) * 100).toFixed(2)}%)
                </span>,
                ...Object.entries(x.num_affected_cases_by_project)
                  .map(([k, v]) => `${k}: ${v} (${((v / cases.hits.total) * 100).toFixed(2)}%)`),
              ]}
            />
          ),
